...
export function ToolRail({ compact = false }: { compact?: boolean }) {
  const tool = usePlanner((s) => s.tool)
  const tools = compact ? TOOLS.filter((t) => t.id !== 'stamp') : TOOLS

  return (
    <aside className="rail">
      <div className="rail-brand" title="ATRIUM">
        A
      </div>
      ...
      <div className="rail-edit">
        ...
        <button type="button" title="Duplicate" aria-label="Duplicate" onClick={() => usePlanner.getState().duplicateSelected()}>
          <Icon name="copy" />
        </button>
